...
TEST(BlockInit, cct1) {
  ...
  EXPECT_EQ(c->logic_blocks[0]->west_conns->size(), 0);

  // top edge
  EXPECT_EQ(c->logic_blocks[1]->north_conns->size(), 0);
  ...
  EXPECT_EQ(c->logic_blocks[15]->south_conns->size(), 0);
  EXPECT_EQ(c->logic_blocks[15]->east_conns->size(), 0);
  EXPECT_EQ(c->logic_blocks[15]->west_conns->size(), 8);

  // top right corner
  EXPECT_EQ(c->logic_blocks[3]->north_conns->size(), 0);
  EXPECT_EQ(c->logic_blocks[3]->south_conns->size(), 8);
  EXPECT_EQ(c->logic_blocks[3]->east_conns->size(), 0);
  ...
  EXPECT_EQ(c->logic_blocks[12]->south_conns->size(), 0);
  EXPECT_EQ(c->logic_blocks[12]->east_conns->size(), 8);
  EXPECT_EQ(c->logic_blocks[12]->west_conns->size(), 0);
